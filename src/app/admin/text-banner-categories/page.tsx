...
export default function TextBannerCategoriesPage() {
  ...
  async function handleAdd() {
    setFormError("");
    if (!isValidCode(form.code)) { setFormError("code는 영문 소문자와 숫자만 허용됩니다."); return; }
    if (!form.name) { setFormError("이름을 입력하세요."); return; }
    setLoading("add");
  ...
  async function handleDelete() {
    ...
    setDeleteError("");
    setLoading("delete");
    const res = await fetch(`/api/admin/text-banner-categories/${selected.id}`, { method: "DELETE" });
    setLoading(null);
    if (!res.ok) { const data = await res.json(); setDeleteError(data.message); return; }
    setModal(null);
    router.refresh();
    fetch("/api/admin/text-banner-categories")
      .then((r) => r.json())
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              <th className="px-4 py-3 text-left font-semibold text-gray-600">순서</th>
              ...
              <tr key={cat.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => handleReorder(cat.id, "up")} disabled={idx === 0 || loading !== null}
                      className="p-1.5 rounded hover:bg-gray-100 disabled:opacity-30 transition-colors cursor-pointer">
                      <ChevronUp className="h-4 w-4" />
                    </button>
                    <button onClick={() => handleReorder(cat.id, "down")} disabled={idx === categories.length - 1 || loading !== null}
                      className="p-1.5 rounded hover:bg-gray-100 disabled:opacity-30 transition-colors cursor-pointer">
        ...
        <Modal title="카테고리 추가" onClose={() => setModal(null)}>
          <div className="space-y-4">
            <div>
              ...
              <input type="text" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="예: promotion" className={inputClass} />
            </div>
            <div>
              <label className="mb-1.5 block text-sm font-medium text-gray-700">카테고리명</label>
              <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="예: 텔레그램 홍보방 TOP 10" className={inputClass} />
            </div>
        ...
        <Modal title="카테고리 삭제" onClose={() => setModal(null)}>
          <div className="space-y-4">
            <div className="rounded-lg bg-red-50 border border-red-200 px-4 py-3">
              <p className="text-sm font-semibold text-eliminate mb-1">⚠️ 주의</p>
              <p className="text-sm text-gray-700">하위에 링크모음이 모두 삭제됩니다. 정말로 삭제합니까?</p>
              <p className="text-sm font-medium text-gray-800 mt-2">
                대상: <span className="text-primary">{selected.name}</span>{" "}
                (<span className="font-mono">{selected.code}</span>)
              </p>
            </div>
            {deleteError && <p className="text-sm text-eliminate">{deleteError}</p>}
