      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b-2 border-primary text-center">
            ...
            <th className="py-3 px-2 w-24">작성자</th>
            <th className="py-3 px-2 w-24">작성날짜</th>
        ...
        <tbody>
          {posts.length === 0 ? (
            ...
            posts.map((post, idx) => {
              ...
              return (
                <tr key={post.id} className="border-b border-gray-200 hover:bg-gray-50 text-center">
                  <td className="py-3 px-2 text-gray-500">{rowNum}</td>
                  <td className="py-3 px-2 text-left">
                    <Link href={`${basePath}/${post.id}`} className="hover:text-secondary">
                      {post.title}
                    </Link>
                  </td>
                  <td className="py-3 px-2 text-gray-600">{post.author_nickname}</td>
                  <td className="py-3 px-2 text-gray-500">{formatDate(post.created_at)}</td>
